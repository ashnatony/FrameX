...
async function testPixazo() {
  ...
  try {
    const urls = [
      ...
      'https://gateway.pixazo.ai/flux/v1/flux-request',
      'https://gateway.pixazo.ai/flux-1-schnell/v1/flux-1-schnell-request',
      'https://gateway.pixazo.ai/studio-ghibli-api/v1/studio-ghibli-api-request'
    ];

    for (const url of urls) {
      log += `\nTesting URL: ${url}\n`;
      try {
        const payload = { prompt: "sketch of a cat" };
  ...
  fs.writeFileSync('test_results2.txt', log);
}
